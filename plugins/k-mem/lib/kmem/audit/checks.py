...
from ..notes.decisions_index import foreign_attribution, render_index
...
def check_adr_index_fresh(root: Path, config: Config | None = None, allow: Allowlist | None = None, today=None) -> CheckResult:
    ...
    # Another generator owns this file. Comparing against our own render would
    # call its correct output stale, which is a collision reported as a defect.
    attribution = foreign_attribution(on_disk)
    if attribution is not None:
        return skip(key, title, f"decisions.md is generated by something else ({attribution or 'unattributed marker'})")
    if rendered != on_disk:
